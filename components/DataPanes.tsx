...
/**
 * Why the files go in this order.
 *
 * For the first four this isn't presentation: each step reads reference data
 * the previous one wrote. Importing sales before attendance produces sales with
 * no closing credit, and nothing in the diff would tell you that had happened.
 *
 * Scroll is last for the opposite reason — it depends on nothing and nothing
 * depends on it. It is a measurement of the page, not of the funnel, and the
 * only thing it needs is a round whose dates it overlaps.
 */
const WHY_HERE: Record<SourceKey, string> = {
  ...
  scroll:     "Optional, and out of the chain — it depends on no other file. Clarity's Scroll export goes here; its date range picks the round, and This round reads the curve against that round's Lead Gen %.",
};
          ...
          return (
            <div key={key} className={key === nextUp ? "src-wrap next" : "src-wrap"}>
              ...
              <ImportUploader
                ...
                kind={spec.kind}
                client={client}
                expects={spec.expects ?? spec.fields.map((f) => f.field).join(" · ")}
              />
              {/* Only the ads step. Meta knows what was spent and nothing else
                  here — leads come from the CRM, sales from the payments file,
                  and a button offering to fetch those would be offering to
                  overwrite better numbers with worse ones. */}
              {key === "ads" ? <MetaPullButton client={client} /> : null}
              {/* No template where a blank one would teach the wrong shape: a
                  Clarity export is generated by Clarity, and a hand-filled
                  imitation of it is a file nobody should ever be making. */}
              {spec.template === false ? null : (
